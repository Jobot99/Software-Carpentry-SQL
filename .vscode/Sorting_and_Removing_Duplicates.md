...
- You can sort on several columns at once with the order of the sort commands deciding priority. For example, this query will sort by "taken" first, then by "person" for rows with the same "taken" value: "SELECT taken, person, quant FROM Survey ORDER BY taken ASC, person DESC;"
